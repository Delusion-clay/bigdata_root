package com.cn.wifi.spark.common.convert

import java.util
import com.cn.wifi.common.convert.BaseDataConvert
...
object DataConvert extends Serializable with Logging {
  ...
  def strMap2esObjectMap(map: java.util.Map[String, String]): java.util.Map[String, Object] = {
    ...
    //{rksj=long, latitude=double, imsi=string, accept_message=string, phone_mac=string, message_time=long, device_mac=string, filename=string, table1=string, phone=string, absolute_filename=string, device_number=string, imei=string, id=string, collect_time=long, send_message=string, object_username=string, username=string, longitude=double}
