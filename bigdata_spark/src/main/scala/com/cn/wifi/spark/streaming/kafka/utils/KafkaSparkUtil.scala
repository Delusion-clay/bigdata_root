package com.cn.wifi.spark.streaming.kafka.utils
...
class KafkaSparkUtil(isUpte:Boolean=true) extends Serializable with Logging {
  ...
  def getMapDSwithOffset(ssc: StreamingContext,
                         ...
                         groupId: String,
                         topic: String): DStream[java.util.Map[String, String]] = {
    ...
    val dsMap = DS.map(kafkaString=>{
      ...
      //转map
      val map = JSON.parseObject(jsonString,new TypeReference[java.util.Map[String, String]](){})
